package NBCamp.Week05.thread.Task;
...
public class Main {
    public static void main(String[] args) {
        ...
        // 1. 카테고리가 여행인 책 제목 조회
        bookList.stream().filter(book -> book.getCategory().equals("여행")).forEach(f -> System.out.println("카테고리가 여행인 책 제목 : " + f.getBookName()));
        System.out.println("");

        // 2. 가격이 16200원 이하인 책의 제목 조회
        bookList.stream()
                .filter(book -> book.getPrice() <= 16200)
                .forEach(f -> System.out.println("가격이 16200원 이하인 책의 제목 : " + f.getBookName()));

        // 3. 책 제목에 "경제"라는 용어가 들어간 책의 목록 조회
        bookList.stream()
                .filter(book -> book.getBookName().contains("경제"))
                .forEach(f -> System.out.println("책 제목에 경제가 포함된 책 제목 : " + f.getBookName()));

        // 4. 가격이 가장 비싼 책의 가격을 조회
        double maxPrice = bookList.stream().mapToDouble(book -> book.getPrice()).max().getAsDouble();
        System.out.println("책 목록 중 가장 비싼 금액은 : " + maxPrice);

        // 5. 카테고리가 it 인 책들의 가격 합 조회
        double sum = bookList.stream().filter(book -> book.getCategory().equals("IT"))
                .mapToDouble(Book::getPrice)
                .sum();
        System.out.println("카테고리가 it 인 책들의 가격 합 : " + sum);

        // 6. 책 할인 이벤트
        // 카테고리가 it인 책들의 가격을 40% 할인하여 새로운 책 리스트 만들기, discountedBookList
